...
//    insertAndSetFirstWall(&head, 2,  520, 440, 120, 10);
    ...
    while(!done){
        ...
        if (robot.auto_mode == 1)
            robotAutoMotorMove(&robot, front_left_sensor, bottom_right_sensor);
...
//        }
        ...
        //Otherwise compute sensor information
            ...
            bottom_right_sensor = checkRobotSensorBottomRightAllWalls(&robot, head);
            if (bottom_right_sensor>0)
                printf("Getting close on the bottom right. Score = %d\n", bottom_right_sensor);
        }
